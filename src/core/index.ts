...
class Core implements ICore {
  ...
  async loadNoteProvingUtils(): Promise<void> {
    ...
    if (isNode) {
      ...
      const zkeyPath = path.resolve(__dirname, "./verifyNoteOwnership_10_0001.zkey");

      this.#noteProvingWasm = await fs.readFile(wasmPath);
      this.#noteProvingZkey = await fs.readFile(zkeyPath);
    } else {
      this.#noteProvingWasm = (await import("./verifyNoteOwnership_10.wasm?url")).default;

      this.#noteProvingZkey = (await import("./verifyNoteOwnership_10_0001.zkey?url")).default;
    }
  }
