...
class CommandPlanner {
  ...
  #generatePlanToUpgradeAddressToNote = (balanceEntry: BalanceEntry): CurvyPlan => {
  ...
  #chunk = (array: Array<any>, chunkSize: number) => {
  ...
  #generateAggregationPlan = (intendedAmount: bigint, items: CurvyPlan[]): CurvyPlanFlowControl => {
    ...
    const chunks = this.#chunk(items, MAX_INPUT_NOTES_PER_AGGREGATION);
    return {
      type: "serial",
      items: [
        {
          type: "parallel",
          items: chunks.map((item) => this.#generateAggregationPlan(intendedAmount, item)),
        },
        {
          type: "command",
          name: "aggregator-aggregate",
  ...
  generatePlan = (balances: BalanceEntry[], intent: CurvyIntent): CurvyPlanFlowControl => {
    ...
    for (const balanceEntry of balances) {
      ...
      plansToUpgradeNecessaryAddressesToNotes.push(this.#generatePlanToUpgradeAddressToNote(balanceEntry));
    }
    ...
    const aggregationPlan = this.#generateAggregationPlan(intent.amount, plansToUpgradeNecessaryAddressesToNotes);
