...
import inputData from "./debug-input.json";
...
const getNoteOwnershipData = async (publicNotes: PublicNote[], s: string, v: string, ownerBabyJubjubPublicKey: string) => {
...
test("verifySignature", async () => {
    ...
    const {
        ...
        message,
        babyJubPubKey,
    } = inputData.verifySignature;

    const verified = verifySignature(BigInt(message), {
...
test("noteOwnershipData", async () => {
    const {
        s,
        v,
        ownerbabyJubjubPublicKey,
        publicNote,
    } = inputData.noteOwnershipData;
