...
test("Decode note shared secret", async () => {
  ...
  const recipientNoteData = await core.sendNote(keyPairs.S, keyPairs.V, {
...
test("Scan notes", async () => {
  ...
  const core = new Core();
  const keyPair1 = await core.generateKeyPairs();
  const keyPair2 = await core.generateKeyPairs();

  const {
    S: ownerS,
    V: ownerV,
    babyJubjubPublicKey: ownerBabyJubjubPublicKey,
  } = await core.getCurvyKeys(keyPair1.s, keyPair1.v);

  const {
    S: otherS,
    V: otherV,
    babyJubjubPublicKey: otherBabyJubjubPublicKey,
  } = await core.getCurvyKeys(keyPair2.s, keyPair2.v);

  const notes: Note[] = [];
  for (let i = 0; i < NUM_NOTES; i++) {
    const isOwnedNote = i < NUM_VALID_NOTES;
    const recipientNoteData = await core.sendNote(isOwnedNote ? ownerS : otherS, isOwnedNote ? ownerV : otherV, {
...
test("Scan owned notes", async () => {
  ...
  const ownedNotes = await core.getNoteOwnershipData(publicNotes, keyPair1.s, keyPair1.v);
...
test("Generate note ownership proof", async () => {
  ...
  const core = new Core();
  const keyPair = await core.generateKeyPairs();

  const {
    S: ownerS,
    V: ownerV,
    babyJubjubPublicKey: ownerBabyJubjubPublicKey,
  } = await core.getCurvyKeys(keyPair.s, keyPair.v);

  const notes: any = [];
  for (let i = 0; i < NUM_NOTES; i++) {
    const recipientNoteData = await core.sendNote(ownerS, ownerV, {
  ...
  const ownedNotes = await core.getNoteOwnershipData(publicNotes, keyPair.s, keyPair.v);
